...
describe("Custom Changelog Generator", () => {
  ...
  beforeEach(async () => {
    ...
    getInfoFromPullRequest = githubInfo.getInfoFromPullRequest as ReturnType<
      typeof vi.fn
    >;
  });
  ...
  describe("getDependencyReleaseLine", () => {
    it("should throw error when repo option is missing", async () => {
      ...
      ).rejects.toThrow("Please provide a repo to this changelog generator");
    });
  ...
  describe("getReleaseLine", () => {
    it("should throw error when repo option is missing", async () => {
      ...
      await expect(
        changelogFunctions.getReleaseLine(changeset, "patch", null),
      ).rejects.toThrow("Please provide a repo to this changelog generator");
    ...
    it("should handle both PR and commit links", async () => {
      ...
      expect(result).toContain(
        "[`newcomm`](https://github.com/owner/repo/commit/newcommit123)",
      );
      expect(result).toContain("Fix: Fixed a bug");
    });
